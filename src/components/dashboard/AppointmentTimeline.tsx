...
export const AppointmentTimeline: FC = () => {
  ...
  const getSlotColor = (duration: number) => {
    if (duration <= 15) return "bg-green-50 border-green-200 text-green-700";
    if (duration <= 30) return "bg-blue-50 border-blue-200 text-blue-700";
    ...
    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-full flex flex-col">
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex flex-col items-end gap-2">
          <label className="text-xs font-medium text-gray-600">Next Slot Type</label>

          <div className="relative">
            <select
              value={selectedVisitType}
              onChange={(e) => setSelectedVisitType(e.target.value as VisitType)}
              className="appearance-none bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-32 p-2 pr-8"
            >
          ...
          <div key={idx} className="relative pl-8 group">
            ...
            <div
              className={clsx(
                ...
                item.status === 'upcoming' && "border-gray-300 group-hover:border-primary",
                item.status === 'cancelled' && "border-red-300 bg-red-50"
              )}
            />

            {item.status === 'current' && (
              <div className="absolute -left-[9px] top-6 w-0.5 h-full bg-primary/20 -z-0" />
            )}
            ...
            >
              ...
              {item.status === 'completed' && (
                <div className="text-green-700 text-xs flex items-center gap-1 border-t pt-2">
                  <CheckCircle2 className="w-3 h-3" /> Checked In
                </div>
              )}

              {item.status === 'current' && (
                <div className="text-primary text-xs flex items-center gap-1 border-t pt-2 animate-pulse">
